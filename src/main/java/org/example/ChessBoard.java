...
public class ChessBoard {
            ...
            if (board[startLine][startColumn].canMoveToPosition(this, startLine, startColumn, endLine, endColumn)) {
                ...
                if (
                        ((startLine == 0 && startColumn == 0)
                                || (startLine == 0 && startColumn == 7)
                                || (startLine == 7 && startColumn == 0)
                                || (startLine == 7 && startColumn == 7)
                                && board[startLine][startColumn] instanceof Rook && board[startLine][startColumn].check)
                                || ((startLine == 0 && startColumn == 4 || startLine == 7 && startColumn == 4) && board[startLine][startColumn] instanceof King && board[startLine][startColumn].check)
    ...
    public boolean isCorrectNewPosition(int line, int column, int toLine, int toColumn) {
        if (line == toLine && column == toColumn) {
            return false;
        }

        if (board[toLine][toColumn] != null && board[toLine][toColumn].getColor().equals(board[line][column].getColor())) {
            return false;
        }
    ...
    public boolean castling0() {
        if (nowPlayer.equals("White")) {
            ...
            if (board[7][0].getSymbol().equals("R") && board[7][4].getSymbol().equals("K") && board[7][1] == null && board[7][2] == null && board[7][3] == null) {
                if (board[7][0].getColor().equals("White") && board[7][4].getColor().equals("White") && board[7][0].check && board[7][4].check && !(new King("White").isUnderAttack(this, 7, 2))) {
                    board[7][4] = null;
